...
name: ADR Generator
description: >
  Generate a numbered Architectural Decision Record (ADR) in /docs/adr/ that
  captures context, decision, alternatives, and consequences using coded bullet
  sections (POS/NEG/ALT/IMP/REF) and YAML front matter. Auto-assigns the next
  4-digit ADR number. Use when the user says "write an ADR", "document this
  decision", "create a decision record", "ADR for X", or asks to formally
  record a technical choice with its rationale, trade-offs, and rejected
  alternatives.
---
